...
print(All.Means$Color.Score)


```
Color score versus PAM plotting 

```{r}

all_trial_data <- read.csv("../Data/20220803-color-PAM-annexin-trial-datasheet.csv")
PAM_plot <- ggplot(all_trial_data, aes(x=color_estimate, y = Fo, colour=color_estimate)) +
  geom_point() +
  xlab("Treatment") + 
  ylab("Fo") + # y-axis label and y axis range
  theme_classic()

PAM_color_score_plot <- ggplot(all_trial_data, aes(x=color.score, y = Fo, colour=color_estimate)) +
  geom_point() +
  xlab("Color score") + 
  ylab("Fo") + # y-axis label and y axis range
  theme_classic()
